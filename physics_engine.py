...
def get_ahta_sensitivity(dist, freq='100kHz'):
    """Límites de detección (DOD) según manual WFRD (18.4 ft para 100kHz)."""
    dod_limits = {'100kHz': 18.4, '400kHz': 12.0, '2MHz': 6.0}
    limit = dod_limits.get(freq, 10.0)
    # Sensibilidad exponencial para modelado proactivo
    return np.exp(-abs(dist) / (limit / 2.8)) if abs(dist) <= limit else 0

def calculate_forward_model(rh, rv, inc, dip, dist):
    """Modelo Forward: Simula la respuesta de resistividad azimutal."""
    try:
        lam = np.sqrt(float(rv) / (float(rh) + 1e-6))
        rel_angle = np.radians(abs(float(inc) - float(dip)))
        sens = get_ahta_sensitivity(dist, '100kHz')
        # Efecto de 'Horns' por anisotropía en la frontera (AHTA Logic)
...
def calculate_tst(thickness, dip):
    ...
    return abs(thickness * np.cos(np.radians(dip)))
